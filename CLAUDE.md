...
5. **Home Assistant push** (`starpilot/system/ha_pushd.py`, registered as `ha_pushd` in `system/manager/process_config.py`: always-run, nice 19). Pushes position, speed, drive state (driving/parked) and ignition to HA's mobile_app webhook. Config is `/data/ha_push.json` = `{"url", "webhook_id"}` on the device, never in the repo. Immediate push on drive↔park / ignition edges, else every 15 s onroad and 10 min offroad; the park transition is what makes "arrived" work before the hotspot dies. HA answers 200-empty for an unknown webhook id, so the sender probes `get_config` first; retries back off 2→60 s and log once per streak; monotonic clock only (the device wall clock is bogus at boot). `update_location` speed is m/s per HA's spec; the km/h sensor is separate. Tests: `starpilot/system/tests/test_ha_pushd.py`. **Never commit `/data/ha_push.json`: this repo is public and a mobile_app webhook can call any HA service.** It lives outside `/data/openpilot`, so updates and branch switches don't touch it; only a factory reset / `/data` wipe does. The webhook id's source of truth is HA's own store (mobile_app entry "Car", app `openpilot.ha_pushd`, entry_id `01M3AY93JT7WTJT9ZR23QT3YTZ`), so recovery from the Mac is one command:
   ```
   docker exec homeassistant python3 -c "import json; d=json.load(open('/config/.storage/core.config_entries')); e=next(e for e in d['data']['entries'] if e['domain']=='mobile_app' and e['data'].get('app_id')=='openpilot.ha_pushd'); print(json.dumps({'url':'https://ha.almirex.com','webhook_id':e['data']['webhook_id']}))" | ssh comma 'umask 077; cat > /data/ha_push.json'
   ```
   The daemon re-reads the file within 60 s; no restart needed. If the HA device was deleted, register a new one via `POST /api/mobile_app/registrations` (bearer token in the home-assistant project's `ha-access-token` memory) and run the same command.
